...
public:
  ...
  Vector4d orientation; //current orientation
  ...
  //Gravitation
  double G = 6.673e-6f;//should be e-11
  ...
  void updatePosition(double timeStep){
     ...
     ***************/
		...
		COM = COM + comVelocity * timeStep;
		RowVector4d rq;
		rq << 0.0, 0.5 * angVelocity * timeStep;
		orientation += QMult(rq, orientation);
  ...
  void updateImpulseVelocities(){
     ...
     ***************/
		...
		for each (Impulse imp in currImpulses)
		{
			RowVector3d aa = imp.second;
			comVelocity = comVelocity + (aa / totalMass);
			RowVector3d contP = imp.first;
			RowVector3d R = contP - COM;
			RowVector3d p = R.cross(imp.second);
			angVelocity = angVelocity + (getCurrInvInertiaTensor()*p.transpose()).transpose();
  ...
  void updateVelocity(double timeStep, double dragCoeff){
	...
	RowVector3d gravities= RowVector3d(0.0,0.0,0.0);
    //integrating external forces (only gravity)
	
	for (int m = 0; m < meshes.size();m++) {
		if (meshes[m]->COM != COM) {
			RowVector3d u = COM - meshes[m]->COM;
			double r = u.norm();
			u.normalize();
			
			gravities += G *(meshes[m]->totalMass / (r*r))*-u;
		}
	}
	//std::cout << "mass:" << totalMass << endl;
	std::cout << "gravity:" << gravities << endl;
    //RowVector3d gravity; gravity<<0,-9.8,0.0;
	//RowVector3d dragForce = -dragCoeff *comVelocity;
	//RowVector3d dragForceAng = -dragCoeff * angVelocity;

    comVelocity+=(gravities)*timeStep;
	//angVelocity += dragForceAng * timeStep;
  }
...
public:
   ...
   *********************************************************************/
  void handleCollision(Mesh& m1, Mesh& m2,const double& depth, const RowVector3d& contactNormal,const RowVector3d& penPosition, const double CRCoeff, double frictionCoeff){
    ...
    if (m1.isFixed){
      /***************
       TODO +
       ***************/
		m2.COM = m2.COM + contactNormal * depth;

    } else if (m2.isFixed){
      /***************
       TODO +
       ***************/
		m1.COM = m1.COM - contactNormal * depth;
    } else { //inverse mass weighting
      /***************
       TODO +
       ***************/
		m2.COM = m2.COM + contactNormal *((depth*m1.totalMass)/ (m2.totalMass+m1.totalMass));
		m1.COM = m1.COM - contactNormal *((depth*m2.totalMass) / (m2.totalMass + m1.totalMass));
    }
     ...
     ***************/
	 ...
	 // inverse inertia tensors for m1 and m2
	Matrix3d m1InvInertia = m1.getCurrInvInertiaTensor();
	Matrix3d m2InvInertia = m2.getCurrInvInertiaTensor();
	// collision arms for m1 and m2
	RowVector3d contactPoint = (penPosition + depth * contactNormal);
	RowVector3d m1ColArm = contactPoint- m1.COM;
	RowVector3d m2ColArm = contactPoint - m2.COM;

	double coeff = (-1)*(1 + CRCoeff);
	
	RowVector3d Vel1 = m1.comVelocity + m1.angVelocity.cross(m1ColArm);
	RowVector3d Vel2 = m2.comVelocity + m2.angVelocity.cross(m2ColArm);

	double VelocityNormal = (Vel1 - Vel2).dot(contactNormal);
	//double VelocityNormal = 0;
	double totalMass = (1 / m1.totalMass + 1 / m2.totalMass);
	//double totalMass = 0;
	double augMassInM1 = (((m1ColArm.cross(contactNormal)))*(m1InvInertia)).dot(m1ColArm.cross(contactNormal).transpose());
	//double augMassInM1 = 0;
	double augMassInM2 = (((m2ColArm.cross(contactNormal)))*(m2InvInertia)).dot(m2ColArm.cross(contactNormal).transpose());
	//double augMassInM2 = 0;

	double j = (coeff*VelocityNormal) / (totalMass + (augMassInM1 + augMassInM2));
	//double j = 0;

	//calculate friction vector, probably not working
	//RowVector3d speedDiff = (m1.comVelocity - m2.comVelocity);
	//RowVector3d frictionOne = (contactNormal.cross(speedDiff));
	//RowVector3d frictionVector = frictionOne.cross(contactNormal);
	//frictionVector.normalize();

	RowVector3d impulse = j * (contactNormal);
	//if (frictionCoeff != 0 && m1.comVelocity.cross(m2.comVelocity)!=RowVector3d(0.0)){
	//	std::cout << "applying fricion" << endl;
	//	std::cout << "fricion" <<frictionVector<< endl;
	//	std::cout << "normal" << contactNormal << endl;
	//	impulse = j * (contactNormal + frictionCoeff * frictionVector);
	//}

	//RowVector3d impulse = RowVector3d(1.0);
    std::cout<<"impulse: "<<impulse<<std::endl;
    if (impulse.norm()>10e-5){
      m1.currImpulses.push_back(Impulse(contactPoint, impulse));
      m2.currImpulses.push_back(Impulse(contactPoint, -impulse));
    }
   ...
   *********************************************************************/
  void updateScene(double timeStep, double CRCoeff, double dragCoeff, double frictionCoeff){
    ...
    for (int i=0;i<meshes.size();i++)
      for (int j=i+1;j<meshes.size();j++)
        if (meshes[i].isCollide(meshes[j],depth, contactNormal, penPosition))
          handleCollision(meshes[i], meshes[j],depth, contactNormal, penPosition,CRCoeff, frictionCoeff);
  ...
  bool loadScene(const std::string dataFolder, const std::string sceneFileName){
	...
	for (int i = 0; i < meshes.size(); i++) {
		for (int j = 0; j < meshes.size(); j++) {
			meshes[i].meshes.push_back(&meshes[j]);
		}
	}
    return true;
  }
